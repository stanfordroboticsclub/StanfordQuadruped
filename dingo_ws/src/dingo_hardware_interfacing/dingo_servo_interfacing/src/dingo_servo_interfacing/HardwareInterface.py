...
class HardwareInterface(link):
    ...
    def set_actuator_postions(self, joint_angles):
        ...
        Parameters
        ----------
        joint_angles : 3x4 numpy array of float angles
        """
        
        servo_angles = self.joint_angles_to_servo_angles(joint_angles)

        print('Final angles for actuation: ',servo_angles)
    
        for leg_index in range(4):
            for axis_index in range(3):
                self.kit.servo[self.pins[axis_index,leg_index]].angle = servo_angles[axis_index,leg_index]
    ...
    def joint_angles_to_servo_angles(self,joint_angles):
        ...
        Returns
        -------
        servo_angles: 3x4 numpy array of float angles
        ...
        for leg in range(4):
            ...
            #Adding offset from IK angle definition to servo angle definition
            self.servo_angles[0,leg] = joint_angles[0,leg]
            self.servo_angles[1,leg] = m.pi/2- THETA2
            self.servo_angles[3,leg] = m.pi/2 + m.pi-THETA0

        # Adding final physical offset angles from servo calibration and clipping to 180 degree max
            calibrated_angles = np.clip(servo_angles + self.physical_calibration_offsets,0,180)
